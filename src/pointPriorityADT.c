...
/*
Item newn(int datum, char*info, int infolen){
...
Item new(int datum, char*info){
	return newn(datum, info, strlen(info));
}
...
void n_insert(struct node *newn_node, struct node *existing, unsigned long n){ //helper for insert
	...
	if (newn_node->data->datum > existing->data->datum){
...
void n_print_PQ(struct node *n){ //helper for print_PQ
	printf("(%d~%s:", n->data->datum, n->data->info);
...
void n_chop(struct node **p){  //helper for extract
	...
	if (n->right==NULL && n->left==NULL){ //if we extract from a node who doesn't have children, it is deleted
		...
		*p=NULL;
		...
		n->data = n->left->data;
		n_chop(&n->left);
	} else if (n->right->data->datum > n->left->data->datum) { //if both children exist, extract from the greater
		n->data = n->right->data;
		n_chop(&n->right);
	} else {
		n->data = n->left->data;
